...
Section~2 sets out the model, Section~3 derives the measurement result, Section~4
states what is proved about uniqueness, and Section~5 confronts the six facts.

\section{Model}

\subsection{Set-Up}

\paragraph{Agents and markets.} There are $N$ countries and $K$ sectors; a
\emph{market} is one sector in one destination country. Three kinds of actors: a
representative \emph{household} in each country, which supplies labour $L_n$
inelastically and spends its income across markets (Cobb--Douglas weights $\beta_k$
across sectors, CES within a market with elasticity $\sigma$; substitution across
markets is $\eta$, with $\sigma>\eta\ge1$); \emph{ownership groups} $g$ ---
multinational parents with a potential factory in every country, and single-factory
local firms --- each headquartered somewhere and owned by countries in shares
$\theta_{gn}$; and the \emph{factories} they operate, which produce differentiated
varieties and sell them across borders. The pool of potential parents grows with
country size, while the capability distribution they draw from is identical
everywhere --- so rich countries host more parents only because more draws have a
better maximum.
\src{The two-tier demand is Atkeson and Burstein (2008); the granular pool of
parents is Gaubert and Itskhoki (2021).}

\paragraph{Technology and delivered costs.} Factory $a$ (head office $h$, located in
$\ell$, selling to $n$, sector $k$) delivers at unit cost
\begin{equation}
...
$\alpha_k$ penalises stand-alone locals and rewards group capability), never a
foreign wage inside a domestic cost --- the choice that decides whether the model has
one solution, and improves the fit besides.
\src{$\gamma_{h\ell}$: Ramondo--Rodr\'iguez-Clare (2013), measured by Head--Mayer
(2019). Shipping from the factory: Tintelnot (2017). Inputs: Caliendo--Parro (2015).
$\alpha_k$: Antr\`as (2003), Antr\`as--Helpman (2004), used on the capability side.}

\paragraph{Entry.} Serving market $n$ costs $F_{a,n}=f_k\,w_\ell$ per period, paid in
the same labour as production --- so entry costs are real resources, they appear in
labour demand, and distributed profits are net of them. Which factories exist is an
outcome, not a list.
\src{Tintelnot (2017).}

\paragraph{Ownership and income.} With $\theta_{gn}$ the share of group $g$ owned by
country $n$,
\begin{equation}
...
\paragraph{Timing.} The model is static. Within the period: capability draws are
realised; given wages, prices and market sizes, each group chooses which factories
operate in which markets; the operating factories compete in every market; and wages,
incomes and price indices adjust until all markets clear. Nothing is sequential in
equilibrium.

\subsection{Markets: Markups, Profits, and Shares}

\paragraph{The mechanism and the markup.} A group expanding one factory also steals
sales from its \emph{own} other factories, and it counts that loss. So the bigger the
group, the more it holds back:
\begin{equation}
...
\label{e:mu}
\end{equation}
where $S_g$ is the group's share of market sales. Both forms rise from
$\sigma/(\sigma-1)$ at $S=0$ toward $\eta/(\eta-1)$ at $S=1$; every result below holds
for \emph{any} increasing $\mu$, so how firms compete is a parameter, not a
commitment.
...
\paragraph{Profit.} With margin $L(S)\equiv1-1/\mu(S)$ and market spending $E$:
\begin{equation}
\Pi_g=E\,S_g\,L(S_g)
...
\paragraph{One market, one unknown.} With $K_g\equiv\sum_{i\in g}c_i^{1-\sigma}$ (the
group's \emph{strength}), $A\equiv\sum_j p_j^{1-\sigma}$ (how \emph{tough} the market
is), and $\psi(S)\equiv S\,\mu(S)^{\sigma-1}$:
\begin{equation}
...
An equilibrium is a set of wages, price indices, market sizes, incomes, an active set
of factories, and quantities such that every group best-responds in every market,
shares sum to one, no group wants to open or close a factory, price indices are
consistent with costs, every worker is employed, and every country's budget balances.
One labour-market condition is redundant by Walras' law, so one wage is the
numeraire. All accounting identities hold to $10^{-15}$ in the program, at wages far
from equilibrium.

\section{The Measurement Result}

For a country $H$ owning shares $\theta_g$ in a market, with
...
stake by $10\%$, overstates Argentina's and Peru's by $24$--$30\%$ --- the sign
varies owner by owner, which no country-level share can express.
...
paper --- every proof written out, every choice justified where it is made, and a
