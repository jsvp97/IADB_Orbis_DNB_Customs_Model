...
are collected in one table in Section~\ref{sec:notation}.

\smallskip
Nothing is left as ``it can be shown''. Where a claim is \emph{proved}, the proof is
written out. Where a claim is only \emph{checked numerically}, it says so in those
words, and every such claim is collected in Section~\ref{sec:notproved} rather than
left for a reader to discover.

\smallskip
\textbf{Part~I} sets out the question, the notation, and a table showing which piece
of the model comes from which paper. \textbf{Part~II} builds the model equation by
equation. \textbf{Part~III} proves the model has a solution and that it is the only
one. \textbf{Part~IV} derives the result the model was built to produce.
\textbf{Part~V} compares model and data fact by fact. \textbf{Part~VI} states the
limits.
\end{keypoint}
...
\newpage
\part{The question, the notation, and the sources}
...
firms that the country owns. Part~IV proves that this is wrong, shows the direction of
...
\section{Three features the model must have, and why each is unavoidable}
\label{sec:three}
...
possible outcomes --- and Part~III is devoted to solving it.

\section{The six facts the model is asked to explain}
\label{sec:facts}
...
origin countries, 2006--2022. Part~V compares each one with the model.
...
\section{Where every piece of the model comes from}
...
same markup and Part~IV has nothing to say.\\
\addlinespace
...
The central mechanism, which we call \emph{cannibalisation}. Everything in Part~IV
follows from it.\\
\addlinespace
...
multiplicity problem Part~III solves.\\
\addlinespace
...
\newpage
\part{The model, equation by equation}
...
\section{The mechanism: a group competes with itself}
\label{sec:cannibal}
...
Part~III is written for a general $\mu(\cdot)$ satisfying
...
every proof in Part~III would collapse. Both \eqref{eq:muC} and \eqref{eq:muB} satisfy
...
\subsection{\texorpdfstring{$\kappa$}{kappa}: the one number that sets the size of the result}
\label{sec:kappa}
...
correction in Part~IV is proportional to it, so the choice of how firms compete
...
\section{Equation 3: shares add up}
...
\section{Equation 4: strength, and the map from strength to share}
...
unknown. This is why Part~III can prove things that would otherwise be out of reach,
...
Part~III a group choosing which of $n$ factories to operate looks like a choice among
...
\section{Equation 5: what things cost}
...
\section{Equation 6: the cost of getting into a market}
...
\section{Equation 7: who owns the profits}
...
\emph{firm by firm} rather than being one number per country --- and Part~IV shows
...
budget balance, so one wage can be fixed as the unit of account. Part~III shows that
(i)--(iv) each have exactly one solution, and that (v)--(vi) have a solution which is
verified to be unique.

\section{Where the possible firms come from}
\label{sec:pool}
...
\section{Layer 2: which factories operate}
...
\subsection{Why the two standard fixes do not work here}
\label{sec:whyhard}
...
\subsection{The resolution: separate the firm's own effect from its rivals'}
...
Suppose Assumption~\ref{as:mu} holds, and consider entry equilibria in which no
group's share exceeds one half in any market. Then there is \emph{at most one} such
outcome. No order of moves is imposed, no outcome is selected by hand, and groups
continue to compete with themselves. (Existence is a separate matter: entry is in
whole factories, so an exact equilibrium need not exist; the measured near-miss is
reported in Section~\ref{sec:notproved}.)
\end{theorem}
...
(Step~2's map). The one-half hypothesis is a restriction on \emph{equilibrium objects}
--- it is checked at the computed solution, where the largest share is far inside it
--- not a primitive assumption on parameters. The numerical verification below scores
every deviation on the fully re-solved market, which is the stronger notion; the two
agree in every tested case.
...
rises. (This is the standard single-crossing argument; Lemma~\ref{lem:A} is what makes
the candidate set a chain in the first place.)
...
\section{Layers 3 and 4: input prices and incomes}
...
$\nu_k\sum_{k'}\omega_{k'k}\log P_{\ell k'}$, and those weights sum to one across
sectors. Together these bound the error contraction factor by $\max_k\nu_k<1$. The
contraction-mapping theorem then gives exactly one solution and geometric convergence.
\end{proof}
...
\subsection{What uniqueness of wages would require}
...
\subsection{Lemma 3: a wage rise raises every price index, by less than one for one}
...
\subsection{Lemma 4: what a factory's wage bill does when costs move}
...
\subsection{Theorem 4: when the wage vector is unique}
...
\subsection{The second hypothesis cannot be dropped}
...
\subsection{A version of the model in which the second hypothesis is not needed}
\label{sec:simple}
...
$\partial\ln W_n/\partial\ln w_m\ \ge\ \bar E^{(m)}_n$. Gross substitutes, and hence
uniqueness by Proposition~\ref{prop:abh}, follows whenever no market's nominal spending
falls.
\end{theorem}
...
\subsection{Uniqueness the constructive way: the model's own tâtonnement contracts}
...
Let $D$ be a set of wage vectors and suppose that on $D$
\begin{enumerate}
...
$\Psi_\kappa$ contracts Hilbert's metric on $D$ with modulus
\[
  \kappa_B\;=\;\sup_{w\in D}\,\tanh\!\bigl(\Delta(A_\kappa(w))/4\bigr)\;<\;1,
  \qquad
...
and the iteration converges to it geometrically at rate $\kappa_B$ from any start in $D$.
\end{theorem}
...
$\kappa(1-M_{nn})<1$, which is (ii). A strictly positive matrix has finite projective
diameter and, by Birkhoff's theorem, contracts the Hilbert metric by
$\tanh(\Delta/4)<1$; for a differentiable, degree-one homogeneous, order-preserving map
the distance between images is bounded by the supremum of that coefficient along the
segment joining the two points. The projective space with Hilbert's metric is complete,
so the contraction mapping theorem applies.
\end{proof}
...
\subsection{How far the theorem reaches, and what is still only checked}
...
\section{The model really is in general equilibrium}
...
\section{What is \emph{not} proved, and what was done instead}
\label{sec:notproved}
...
\subsection{That equilibrium wages are unique \emph{without conditions}}
...
\subsection{That an exact whole-number entry solution always exists}
...
\section{The result in words, before any algebra}
...
\subsection{The object, measured in the data}
...
\section{Why constant markups make the question disappear}
...
ownership share times total profit. The distribution of ownership across firms is then
irrelevant to income and to the optimal tariff.
\end{proposition}
...
\section{A result that runs against the original hypothesis}
\label{sec:tariff}
...
\section{Which numbers were chosen, and how}
...
\subsection{Adding entry pins down a parameter that used to float free}
...
\section{Fact by fact}

\subsection{Fact 1: international firms are a large share of exports, and foreign ones dominate}
\label{sec:fact1}
...
\subsection{Fact 2: foreign firms specialise in complex products}
\label{sec:fact2}
...
\subsection{Fact 3: parent firms come from a handful of countries}
...
\subsection{Fact 4: grouping factories by their owner raises measured concentration}
...
\begin{keypoint}
\textbf{This is the fact Part~III exists to protect.} The \emph{ordering} is a
...
\subsection{Fact 5: where there are more international firms, local firms export more}
\label{sec:fact5}
...
+ moderate spillovers & $\approx0.28$ & $-43\%$ & $33\to\mathbf{36}$\\
same, $\lambda$ at the data's level & $\mathbf{0.09}$ & $-23\%$ & $20\to\mathbf{29}$\\
\bottomrule
\end{tabular}
\end{center}

\noindent The margin is the first channel that moves the fact at first order on its
own, and it flips the \emph{extensive} margin to the data's sign: with it, adding
international firms \emph{increases} the number of local plant--market export pairs,
as in the data. In the data's units --- per log-unit of the number of international
firms --- the model's response moves from $\approx-2.9$ without the margin to
$\approx-0.2$ at $\lambda=0.09$, against the measured $+0.087$: more than an order of
magnitude of the gap is closed, and what remains is a joint recalibration with the
margin switched on rather than a missing mechanism. That recalibration is the model's
next task, and this diagnosis belongs in the abstract, not a footnote.
...
potential plants so that the entry theorem is untouched, and both measured against the
fact rather than assumed. The outside supplier (\texttt{row\_L}) is the $\lambda$ of
the measurement operator made structural: the origin's exports over the destination's
absorption, identifiable from trade and production data (Comtrade imports plus
domestic production), which is also how its size should be calibrated. It enters as an
ordinary country --- labour, income, budget balance --- so the accounting identities
and every uniqueness theorem carry over without modification.}

\subsection{Fact 6: distance matters less for international firms}
...
5 & international firms raise local exports & \textbf{fails}, and worse with entry\\
6 & distance matters less for international firms & \textbf{produced}, magnitudes too large\\
\bottomrule
\end{tabular}
\end{center}

Three parameters are matched: $f$ to the number of exporters, the head-office
disadvantage to Fact 2's gradient, and $\zeta$ to Fact 1's split. There used to be a
fourth --- a multinational productivity edge fitted to Fact~1's level --- and it is no
longer needed: once head-office services are a capability rather than a factor bought
at the parent's wage (Section~\ref{sec:hqchoice}), multinationals are competitive
enough without it and it is set to zero. Facts 3, 4 and 6 are therefore not fitted ---
the model was not asked to reproduce them and does so anyway.

\newpage
\part{Limits, and what to do next}

\section{What this model does not do}

\begin{itemize}
\item \textbf{Fact 5 is a confirmed failure}: the fact survives the saturated
fixed-effects test at half its published size (Section~\ref{sec:fact5}) and the model
has the wrong sign. This is the only outright failure, and it now needs a mechanism,
not a re-estimation.
...
\section{What to do next, in order --- and what is already done}
...
\item \textbf{Fact 5 has now been re-estimated with destination-by-product-by-year
controls, and it survives at half size} ($0.166\to0.087$ intensive, $0.117\to0.061$
extensive, PPML agreeing; Section~\ref{sec:fact5}). The model therefore needs a genuine
mechanism for the positive sign --- and the spillover knob is already shown not to be
it. This is now the model's foremost open problem.
...
\texttt{facts} & the six facts of Part~V\\
