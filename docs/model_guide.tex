...
\paragraph{Roadmap.} Section~\ref{sec:factsstate} states the six facts the model is
asked to explain. Section~\ref{sec:model} sets out the model: the environment agent
by agent, the market game, and the definition of equilibrium.
Section~\ref{part:proofs} proves existence and uniqueness layer by layer, and states
exactly where proof ends and machine verification begins.
Section~\ref{sec:result} derives the measurement result the model was built to
produce and reports its empirical counterpart. Section~\ref{sec:quant} calibrates
the model and confronts the facts. The final section states the limits. Ancillary
material --- the notation table, the equation summary, and instructions for
reproducing every number --- is in the appendices.
...
\paragraph{Groups, factories, and the pool of potential parents.}
\label{sec:pool}
...
\paragraph{Technology and delivered costs.}
...
\paragraph{Where the head office is, and why it is not in the cost equation.}
\label{sec:hqchoice}
...
\paragraph{Ownership and income.}
...
\paragraph{Timing.} The model is static. Within the period: the pool of
potential groups and every capability draw are realised; given wages, price indices
and market sizes, each group chooses which of its factories operate in which markets
(the entry game of Section~\ref{part:proofs}); the operating factories then compete
--- quantities or prices --- in every market, and production, pricing and trade
occur; and wages, incomes and price indices adjust so that every market in
Definition~\ref{def:eq} clears. Nothing is sequential in equilibrium --- the order
here is the order in which the pieces are easiest to understand, and the order in
which the program solves them.

\subsection{Markets: Markups, Profits, and Shares}

\paragraph{The mechanism: a group competes with itself.}
\label{sec:cannibal}
...
\paragraph{\texorpdfstring{$\kappa$}{kappa}: the one number that sets the size of the result.}
\label{sec:kappa}
...
\paragraph{Strength, and the map from strength to share.}
